...
static lv_obj_t *security_ui_defence_widget_duplicate(lv_obj_t *parent,char *txt,void *img_src)
{
	...
	lv_obj_t *cont = universal_cont_create(&elm_cont, NULL, LV_LAYOUT_COLUMN_MID, LV_FIT_NONE);
	...
	lv_obj_t *label = universal_label_chinese_create(cont, txt, universal_font_size_12);
...
static lv_obj_t *scr_security_ui_defence_mode()
{
	...
	lv_obj_t *bg_obj = universal_obj_create(&elm_obj, NULL);
...
static lv_obj_t *scr_security_ui_head_title(lv_obj_t *parent, char *head_str)
{
	return universal_misc_rm_head_title(parent, head_str);
}
...
{	
	...
	scr_security_ui_head_title(bg, "安防设置");
//return
...
static void scr_security_ui_create()
{
	ui_printf("scr_security_ui_create\n");
	...
	scr_security_ui_head_title(bg, "安防报警");
	//return
	scr_security_ui_return(bg, security_eventcb_return_home);

	uint16_t unsecurity_width = 100, unsecurity_height = 60;
	uint16_t unsecur_align_x = 100, unsecury_align_y = -50;
	uint16_t security_width = unsecurity_width * 2, security_height = unsecurity_height;
	uint16_t align_left_right_space = 50;
	uint16_t security_set_width = security_width, security_set_height = security_height;
	uint16_t align_center_space = 80;

	lv_obj_t *btn_unsecurity = security_ui_btn_create(bg, unsecurity_width, unsecurity_height, "撤防");
	screen_adaption_lvgl_obj_align(btn_unsecurity, NULL, LV_ALIGN_IN_LEFT_MID, unsecur_align_x, unsecury_align_y);

	lv_obj_t *btn_inhome_security = security_ui_btn_create(bg, security_width, security_height, "在家布防");
	screen_adaption_lvgl_obj_align(btn_inhome_security, btn_unsecurity, LV_ALIGN_OUT_RIGHT_TOP, align_left_right_space, 0);

	lv_obj_t *btn_outhome_security = security_ui_btn_create(bg, security_width, security_height, "离家布防");
	screen_adaption_lvgl_obj_align(btn_outhome_security, btn_inhome_security, LV_ALIGN_OUT_RIGHT_TOP, align_left_right_space, 0);

	lv_obj_t *btn_security_set = security_ui_btn_create(bg, security_set_width, security_set_height, "安防设置");
	screen_adaption_lvgl_obj_align(btn_security_set, NULL, LV_ALIGN_CENTER, 0, align_center_space);
	lv_obj_set_event_cb(btn_security_set, security_eventcb_set_btn);
}
...
{
	bool is_true = gui_sys_screen_register(scr_security_ui_create, scr_security_quit, NULL, NULL, NULL, scr_security);
