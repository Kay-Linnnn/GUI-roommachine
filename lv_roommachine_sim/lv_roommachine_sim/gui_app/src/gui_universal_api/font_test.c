...
static uint8_t __g_font_buf[187];//如bin文件存在SPI FLASH可使用此buff

//extern uint32_t LV_USER_FONT_ADDR;
static uint8_t *__user_font_getdata(int offset, int size){
    //如字模保存在SPI FLASH, SPIFLASH_Read(__g_font_buf,offset,size);
    //如字模已加载到SDRAM,直接返回偏移地址即可如:return (uint8_t*)(sdram_fontddr+offset);
    //ui_printf_plus("__user_font_getdata,font_addr:%d",LV_USER_FONT_ADDR);
    fs_test_flash_read(__g_font_buf,offset,size);
    //return (uint8_t *)(LV_USER_FONT_ADDR + 0 + offset);
    return __g_font_buf;
}
