...
static lv_obj_t * setting_ui_head_title_create(char *head_str)
{
...
*/
LV_EVENT_CB_DECLARE(sysset_first_page_return)
{
	if (e == LV_EVENT_CLICKED)
	{
...
static lv_obj_t *system_set_ui_first_page_return_create(lv_obj_t *parent,void *return_free_event)
{
...
*/
static void system_info_page_create()
{
	lv_obj_t *page_bg = NULL;
	...
	_system_setting_data_memory *p_memory = (_system_setting_data_memory *)get_local_pointer(p_data_memory);
#if 0	
	lv_obj_t *label_title = lv_label_create(page_bg,NULL);
	lv_theme_apply(label_title,(lv_theme_style_t)sigma_star_theme_label_chinese);
	lv_label_set_text(label_title,"系统信息");
	lv_obj_align(label_title,NULL,LV_ALIGN_IN_TOP_MID,0,20);
#endif
	system_set_ui_first_page_title_create(page_bg,"系统信息");
	system_set_ui_first_page_return_create(page_bg, system_info_page_free);
#if 0
	static lv_style_t btn_style;
	lv_style_init(&btn_style);
	lv_style_set_bg_color(&btn_style,LV_STATE_DEFAULT,LV_COLOR_GRAY);
	...
	lv_obj_t *return_btn = lv_btn_create(page_bg,NULL);
	lv_obj_set_size(return_btn,120,60);
	lv_obj_align(return_btn,NULL,LV_ALIGN_IN_TOP_RIGHT,0,0);
	//lv_theme_apply(return_btn,(lv_theme_style_t)sigma_star_theme_Btn_home_return);
	lv_btn_set_layout(return_btn,LV_LAYOUT_ROW_MID);
	lv_obj_set_event_cb(return_btn, sysset_first_page_return);
	lv_obj_add_style(return_btn,LV_BTN_PART_MAIN,&btn_style);
	return_btn->user_data = system_info_page_free;
...
static void address_page_free()
{
	...
	free_queue_free(addr_set_free_queue_list);
	addr_set_free_queue_list = NULL;
}

static void address_page_create()
{
	lv_obj_t *page_bg = NULL;
	...
	system_set_ui_first_page_return_create(page_bg, address_page_free);
#if 0
	lv_obj_t *label_title = lv_label_create(page_bg, NULL);
	lv_theme_apply(label_title, (lv_theme_style_t)sigma_star_theme_label_chinese);
	lv_label_set_text(label_title, "地址设置");
	lv_obj_align(label_title, NULL, LV_ALIGN_IN_TOP_MID, 0, 20);
	...
	lv_obj_set_event_cb(return_btn, sysset_first_page_return);
	lv_obj_add_style(return_btn, LV_BTN_PART_MAIN, &btn_style);
	return_btn->user_data = address_page_free;
...
***Prefix format: PwdChg_
***obj event_cb format: PwdChg_EventCB_
*/
...
static void PWdChg_change_pwd_create(lv_obj_t *parent)
{
	...
	system_set_ui_first_page_title_create(page_bg,"密码设置");
...
static void sysset_ui_password_page_create()
{
	lv_obj_t *page_bg = NULL;
	...
	system_set_ui_first_page_return_create(page_bg,NULL);
	...
	lv_obj_align(cont_bg,NULL,LV_ALIGN_CENTER,0,0);
...
LV_EVENT_CB_DECLARE(widget_callback_auto)
{
	if (e == LV_EVENT_CLICKED)
	{
		if (!is_null_ptr(obj->user_data))
...
static lv_obj_t *settting_widget_duplicate(lv_obj_t *parent,void *scr_img,char *label_str)
{
...
static void setting_enter_create()
{
	...
	lv_obj_t *obj_head = setting_ui_head_title_create("系统设置");

	lv_obj_t *return_btn = lv_btn_create(lv_scr_act(), NULL);
...
//widgets
	...
	lv_obj_t *set_widget_scrpro = settting_widget_duplicate(lv_scr_act(), &lovelyghost_6464, "屏幕设置");
	...
	lv_obj_t *set_widget_timer = settting_widget_duplicate(lv_scr_act(), &lovelyghost_6464, "时间设置");
	lv_obj_align(set_widget_timer, set_widget_scrpro, LV_ALIGN_OUT_RIGHT_TOP, left_right_space_up, 0);

	lv_obj_t *set_widget_password = settting_widget_duplicate(lv_scr_act(), &lovelyghost_6464, "密码设置");
	lv_obj_align(set_widget_password, set_widget_timer, LV_ALIGN_OUT_RIGHT_TOP, left_right_space_up, 0);
	lv_obj_set_event_cb(set_widget_password, widget_callback_auto);
	set_widget_password->user_data = sysset_ui_password_page_create;

	lv_obj_t *set_widget_ring = settting_widget_duplicate(lv_scr_act(), &lovelyghost_6464, "铃声设置");
	...
	lv_obj_t *set_widget_root = settting_widget_duplicate(lv_scr_act(), &lovelyghost_6464, "工程设置");
...
/*****
->Below<- Password for entering setting
********/
static void pwd_ta_control(_btnm_ctrl_type ctrl_t,const char *txt,void *get_data)
{
	if (btnm_ctrl_null == ctrl_t) return;
	...
	switch (ctrl_t)
	{
	  case btnm_ctrl_add:
		  ...
		  //lv_textarea_add_char(p_dynamic->pwd_ta,txt);
		break;
	  ...
	  case btnm_ctrl_check:
		 ...
		 //*(char *)get_data = lv_textarea_get_text(p_dynamic->pwd_ta);
		  //ui_printf("get_data:%s\r\n", *(char *)get_data);
		  break;
...
static lv_obj_t* psw_wrong_create()
{
	lv_obj_t *bg_top = lv_obj_create(lv_scr_act(), NULL);
	lv_theme_apply(bg_top, (lv_theme_style_t)sigma_star_theme_default_bg);
...
static void pwd_confirm_control(const char *pwd_txt)
{
	setting_enter_create();
#if 0
	if (pwd_txt != NULL) {
		if (strcmp(pwd_txt, "123456") == 0) {
			...
			lv_obj_t *obj_bg = psw_wrong_create();
...
LV_EVENT_CB_DECLARE(pwd_btnm_event_handle)
{
	...
	if (strcmp(txt, "删除") == 0) {
		pwd_ta_control(btnm_ctrl_del, NULL, NULL);
		return;
	}
	else if (strcmp(txt, "确认") == 0) {
		char *get_txt = NULL;
		ui_printf("get_txt:%s,&get_txt:%p\r\n", get_txt, &get_txt);
		pwd_ta_control(btnm_ctrl_check, NULL, &get_txt);
		//ui_printf("pwd confirm,get_txt:%s\r\n",get_txt);
		pwd_confirm_control(get_txt);
		return;
	}
	else {
		pwd_ta_control(btnm_ctrl_add, txt, NULL);
	}
}

static void system_setting_pwd_title_create(lv_obj_t *parent)
{
...
static void pwd_add_ta(lv_obj_t *obj_ta)
{
...
static void system_setting_pwd_page_create()
{
	lv_obj_t *pwd_bg = get_a_default_bg();
	pwd_page = pwd_bg;

	system_setting_pwd_title_create(pwd_bg);
	...
	pwd_add_ta(obj_ta); //save ta to system setting pwd
	...
	lv_theme_apply(obj_btnm, (lv_theme_style_t)sigma_star_theme_btnmatrix_pwd);
	lv_obj_set_event_cb(obj_btnm, pwd_btnm_event_handle);

	lv_obj_t *obj_btn = lv_btn_create(pwd_bg, NULL);
	lv_obj_set_size(obj_btn, 150, 50);
	lv_obj_set_pos(obj_btn, LV_HOR_RES - 150 - 30, 30);
	lv_theme_apply(obj_btn, (lv_theme_style_t)sigma_star_theme_Btn_home_return);
	lv_obj_set_event_cb(obj_btn, SysSet_return_btn_event_handle);
